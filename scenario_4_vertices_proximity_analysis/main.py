...
def simular_SIR(grafo, beta, gamma, infectados_iniciais, nos_vacinados):
    ...
    estados_temporais.append(estado.copy())  # último estado
...
grafo_pesos_invertidos = copy.deepcopy(grafo)
for u, v in grafo_pesos_invertidos.edges():
    peso_original = grafo_pesos_invertidos[u][v]['peso']
    grafo_pesos_invertidos[u][v]['peso_inverso'] = 1 / peso_original

centralidade = nx.closeness_centrality(grafo_pesos_invertidos, distance='peso_inverso')

beta = 0.7   
gamma = 0.4  
nos_vacinados = sorted(centralidade, key=centralidade.get, reverse=True)[:10]
